...
impl ComponentRegistry {
    ...
    pub fn get(&self, key: &str) -> Option<&Box<dyn ComponentApplier>> { self.map.get(key) }
    pub fn iter(&self) -> impl Iterator<Item = (&'static str, &Box<dyn ComponentApplier>)> { self.map.iter().map(|(k, v)| (*k, v)) }
}
...
// ---- helpers ----
pub fn from_json<T: DeserializeOwned>(v: &Json) -> anyhow::Result<T> { Ok(serde_json::from_value(v.clone())?) }
...
fn parse_hex_color(s: &str) -> Option<Color> {
    ...
    let bytes = match hex.len() { 6 => u32::from_str_radix(hex, 16).ok().map(|v| (v << 8) | 0xFF)?, 8 => u32::from_str_radix(hex, 16).ok()?, _ => return None };
...
impl ComponentApplier for TransformApplier {
    fn key(&self) -> &'static str { "Transform" }
    fn apply(&self, payload: &Json, entity: &mut EntityCommands, _scratch: &mut EntityScratch, _ctx: &mut ApplyCtx) -> anyhow::Result<()> {
        let td: TransformDef = from_json(payload)?;
        let mut t = Transform::IDENTITY;
        if let Some([x, y, z]) = td.t { t.translation = Vec3::new(x, y, z); }
        if let Some([x, y, z]) = td.s { t.scale = Vec3::new(x, y, z); }
        if let Some([x, y, z, w]) = td.r { t.rotation = Quat::from_xyzw(x, y, z, w); }
        if let Some(e) = td.euler {
            t.rotation = Quat::from_euler(EulerRot::YXZ, e.y.to_radians(), e.x.to_radians(), e.z.to_radians());
        }
        if let Some([lx, ly, lz]) = td.look_at { t = Transform::from_translation(t.translation).looking_at(Vec3::new(lx, ly, lz), Vec3::Y); }
        entity.insert(t);
        Ok(())
    }
}
...
impl ComponentApplier for VisibilityApplier {
    ...
    fn apply(&self, payload: &Json, entity: &mut EntityCommands, _scratch: &mut EntityScratch, _ctx: &mut ApplyCtx) -> anyhow::Result<()> {
        match payload.as_str() { Some("Visible") => entity.insert(Visibility::Visible), Some("Hidden") => entity.insert(Visibility::Hidden), _ => entity.insert(Visibility::Inherited) };
        Ok(())
    }
}
...
impl ComponentApplier for StandardMaterialRefApplier {
    ...
    fn apply(&self, payload: &Json, _entity: &mut EntityCommands, scratch: &mut EntityScratch, ctx: &mut ApplyCtx) -> anyhow::Result<()> {
...
impl ComponentApplier for MeshRefApplier {
    ...
    fn apply(&self, payload: &Json, entity: &mut EntityCommands, scratch: &mut EntityScratch, ctx: &mut ApplyCtx) -> anyhow::Result<()> {
...
impl ComponentApplier for SceneRefApplier {
    ...
    fn apply(&self, payload: &Json, entity: &mut EntityCommands, _scratch: &mut EntityScratch, ctx: &mut ApplyCtx) -> anyhow::Result<()> {
