...
use bevy::log::info;
use bevy::window::{RawHandleWrapper, Window, WindowClosed, WindowCreated, WindowWrapper, exit_on_all_closed};
use std::ops::Deref;
use uuid::Uuid;

#[derive(Eq, Hash, PartialEq, Debug, Copy, Clone)]
pub(crate) struct WindowId(Uuid);
impl WindowId { pub fn new() -> Self { WindowId(Uuid::new_v4()) } }
...
impl Deref for AppViewWindow { type Target = AppView; fn deref(&self) -> &Self::Target { &self.0 } }
...
pub fn create_bevy_window(app: &mut App) {
	...
	let view_obj = app.world_mut().remove_non_send_resource::<AndroidViewObj>().unwrap();

	let mut create_window_system_state: SystemState<(Commands, Query<(Entity, &mut Window), Added<Window>>, EventWriter<WindowCreated>, NonSendMut<AppViews>)> = SystemState::from_world(app.world_mut());
	let (mut commands, mut new_windows, mut created_window_writer, mut app_views) = create_window_system_state.get_mut(app.world_mut());

	for (entity, mut bevy_window) in new_windows.iter_mut() {
		if app_views.get_view(entity).is_some() { continue; }
		let app_view = app_views.create_window(view_obj, entity);
		let logical_res = app_view.logical_resolution();
		bevy_window.resolution.set_scale_factor(app_view.scale_factor as f32);
		bevy_window.resolution.set(logical_res.0, logical_res.1);
		info!("bevy_window: {:?}", bevy_window.resolution);
		commands.entity(entity).insert(RawHandleWrapper::new(&app_view.0).unwrap());
