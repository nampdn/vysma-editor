...
impl ModuleLoader {
    ...
    pub fn load_module(
        ...
        if let Some(doc) = self.loaded_modules.get(module_name) {
            return Ok(doc.clone());
        }
        ...
        for (import, dep_doc) in dep_docs {
            self.merge_module_import(&mut doc, &dep_doc, &import)?;
        }
    ...
    pub fn merge_module_import(
        &self,
        target_doc: &mut SceneDoc,
        source_doc: &SceneDoc,
        import: &ModuleImport,
    ) -> anyhow::Result<()> {
        ...
        for entity in &source_doc.entity {
            if let Some(export) = source_doc.exports.iter().find(|e| e.name == import.name) {
                if export.entities.contains(&entity.name.as_ref().unwrap_or(&"".to_string())) {
                    ...
                    if let Some(name) = &mut namespaced_entity.name {
        ...
        for trigger in &source_doc.triggers {
            if let Some(export) = source_doc.exports.iter().find(|e| e.name == import.name) {
                if export.triggers.contains(&trigger.name.as_ref().unwrap_or(&"".to_string())) {
                    ...
                    if let Some(name) = &mut namespaced_trigger.name {
                        *name = format!("{}::{}", namespace, name);
                    }
    ...
    pub fn register_module_path(&mut self, module_name: String, path: String) {
        self.module_paths.insert(module_name, path);
    }

    /// Get all loaded modules
    pub fn get_loaded_modules(&self) -> &HashMap<String, SceneDoc> {
        &self.loaded_modules
    }
    ...
    pub fn get_module_dependencies(&self, module_name: &str) -> Vec<String> {
        if let Some(doc) = self.loaded_modules.get(module_name) {
            doc.modules.iter().map(|m| m.name.clone()).collect()
        } else {
            Vec::new()
        }
    }
...
impl Plugin for ModuleLoaderPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ModuleLoader>();
    }
} 