...
impl ModuleRegistry {
    ...
    pub fn register_module(&mut self, name: String, path: String, doc: SceneDoc) -> anyhow::Result<()> {
        let exports = doc.exports.iter()
            .find(|e| e.name == name)
            .cloned()
            .unwrap_or_else(|| ModuleExport {
                name: name.clone(),
                prefabs: vec![],
                entities: vec![],
                triggers: vec![],
        ...
        let dependencies = doc.modules.iter()
            .map(|m| m.name.clone())
            .collect();
    ...
    pub fn get_public_modules(&self) -> Vec<&RegisteredModule> {
        self.modules.values()
            .filter(|m| m.exports.public)
            .collect()
    }
    ...
    fn visit_module(
        ...
        if temp_visited.contains(module_name) {
            return Err(anyhow::anyhow!("Circular dependency detected: {}", module_name));
        }
        ...
        if let Some(module) = self.modules.get(module_name) {
            for dep in &module.dependencies {
                self.visit_module(dep, resolved, visited, temp_visited)?;
            }
        }
    ...
    fn build_dependency_graph(&mut self) {
        ...
        for (name, module) in &self.modules {
            self.dependency_graph.insert(name.clone(), module.dependencies.clone());
        }
    }
    ...
    pub fn get_module_metadata(&self) -> Vec<ModuleMetadata> {
        self.modules.values()
            .map(|m| ModuleMetadata {
                ...
                description: None,
                category: None,
                public: m.exports.public,
                export_counts: ExportCounts {
                    prefabs: m.exports.prefabs.len(),
                    entities: m.exports.entities.len(),
                    triggers: m.exports.triggers.len(),
                    vars: m.exports.vars.len(),
                },
            })
            .collect()
    }
